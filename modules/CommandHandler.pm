...
sub readConfig{
    ...
    ## Where modules::Publish writes pages, and the URL they are served from.
    ## Config-driven rather than hardcoded so the bot is not pinned to one host.
    $self->{publish_path} = $cfg->param("BotSettings.publish_path")
        || '/var/www/shalerocksbot/publish/';
    ## NOTE: these are read from [BotSettings]. Putting them in any other
    ## section of rocksbot.cfg leaves them silently undef here.
    $self->{publish_base_url} = $cfg->param("BotSettings.publish_base_url")
        || 'https://shalerocksbot.us.to/';
...
sub loadPluginInfo{
    ...
    foreach my $name (@{$self->{plugins}}){
        ...
        $hash->{publish_path} = $self->{publish_path};
        $hash->{publish_base_url} = $self->{publish_base_url};
