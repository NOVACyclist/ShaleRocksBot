...
sub getOutput {
        ...
        || $self->hasFlag("list") )
    {
        ...
        if ( !( $badge_name = $self->hasFlagValue("name") ) ) {
            return (
"A badge name is required.  Example: -name = QuitSmoking  Example: -name=\"Quit Smoking\""
            );
        }
...
"A date is required.  Example: -date=\"10/31/2012\"  Example: -date=\"January 4, 2013\"  Example: -date=\"4 days ago\"  Example: -date=\"last tuesday\""
        ...
        if ( @records > 0 ) {
            return (
"Looks like you already have a badge by that name. Use delete or reset."
            );
        }
        ...
        if ($temp) {
            return
"'$temp' You can't use special characters in your badge name. Only letters and numbers, and spaces if you must.";
        }

        $c->add( $badge_name, $date_db );

        return
          "added badge '$badge_name' with a date of $date_printable for user "
          . $self->{'nick'} . ".";
        ...
        if ( $badge_name = $self->hasFlagValue("name") ) {
            ...
            foreach my $badge (@records) {
                ...
                if ( $self->hasTime($badge_date) ) {
                    $ret .=
                      "$badge_user: " . $self->timeSince($badge_date) . ". ";
        ...
        if ( !( $badge_name = $self->hasFlagValue("name") ) ) {
            return
"You must specify a badge name.  Example:  -name = \"QuitSmoking\" ";
        }

        if ( !( $badge_cost = $self->hasFlagValue("cost") ) ) {
            return
"You must specify a daily cost.  Example:  -cost = 15.99.  To clear the cost, use -cost = none";
        }

        $badge_cost =~
          s/^(\D+)|(\D+)$//;    #remove the unit if there is one and save it.
        ...
        if ( @records == 1 ) {
              ...
              )
            {
                if ( $badge_unit_position eq "AFTER" ) {
                    return
                      "Badge $badge_name updated. $badge_cost$badge_unit / day";

                }
                else {
                    return
                      "Badge $badge_name updated. $badge_unit$badge_cost / day";
...
"You must specify a badge name.  Example:  -name = \"QuitSmoking\" ";
        ...
        if ( @records == 1 ) {
            ...
            return ("Can't find a badge for you with that name.  Use '"
                  . $self->{BotCommandPrefix}
                  . "badge list' to list your current badges." );
        }
        ...
        if ( @records == 1 ) {
            ...
            if ($badge_date) {
                ...
                if ($err) {
                    print "ERROR PARSING DATE : $err\n";
                    return (
"There was an error parsing that date. Badge not updated"
                    );
                }
        ...
        foreach my $badge (@records) {
            ...
            if ( $badge->{'val1'} eq $badge_name ) {
                ...
                if ( $self->hasTime($badge_date) ) {
                    ...
                    if ($badge_dollar) {
                        my $saved =
                          $badge_dollar * $self->daysSince($badge_date);
                        $saved = commify($saved);
                        if ( $badge_unit_position eq "AFTER" ) {
                            $ret .= ' (' . $saved . $badge_unit . ' saved)';
        ...
        if ($ret) {
            ...
            return ( "You don't have a badge by that name. "
                  . $self->help('badge') );
        }
    }
}
...
sub addHelp {
    ...
    $self->addHelpItem( "[plugin_description]",
        "Badges. Get a date-based \"days since\" badge for whatever." );
...
"Use $self->{BotCommandPrefix}badge to list your badges. Flags: -add -delete -update -cost, with -name=\"Badge Name\" -date=\"a date string\".  Use -all to see all users' badges, use -all -name=\"Badge Name\" to see all badges of a particular type for all users. Use -nick=SomeGuy to see SomeGuy's badges"
    ...
    $self->addHelpItem( "[badge][-add]",
"Create a badge.  Usage: $self->{BotCommandPrefix}badge -add -name=\"Badge Name\" -date=\"some date\""
    );
...
"Change the date on a badge.  Usage: $self->{BotCommandPrefix}badge -update -name=\"Badge Name\" -date=\"some date\""
    ...
    $self->addHelpItem( "[badge][-delete]",
        "Usage: $self->{BotCommandPrefix}badge -delete -name=\"Badge Name\"" );
    $self->addHelpItem( "[badge][-cost]",
"Set a daily cost for a badge. Example: $self->{BotCommandPrefix}badge -cost =(\$,\£, etc.)5.25 -name=\"Badge Name\". To clear a cost, use -cost = none"
    );
...
"See system wide badges.  Usage: $self->{BotCommandPrefix}badge -all.  To see all badges of a particular type, use $self->{BotCommandPrefix}badge -all -name=\"Badge Name\""
    ...
    $self->addHelpItem( "[badge][-nick]",
"See another users badges.  Usage: $self->{BotCommandPrefix}badge -nick = someguy"
    );
    $self->addHelpItem( "[badge][-name]",
"See a particular badge.  Usage: $self->{BotCommandPrefix}badge -name = \"Badge Name\""
    );
=======
sub addHelp{
   my $self = shift;
   ...
   $self->addHelpItem("[badge][-cost]", "Set a daily cost for a badge. Example: $self->{BotCommandPrefix}badge -cost =(\$, \£, kcal, A\$, C\$ etc.)5.25 -name=\"Badge Name\". To clear a cost, use -cost = none");
