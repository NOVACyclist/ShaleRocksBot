...
sub getOutput {
   ...
   if ( $google->{$cmd} ) {
      ...
      my $url  = "https://translate.google.com/?sl=auto&tl=" . $google->{$cmd} . "&text=" . uri_escape($options) . "&op=translate";
      my $page = $self->getPage($url);
...
sub listeners {
   ...
   my @commands = [
      'af',     'boston',   'smurf',  'ayb',     'yoda',    'valley', 'piglatin', 'l33t',  'hax0r',   'redneck', 'cockney',  'fudd',
      'bork',   'moron',    'jive',   'english', 'german',  'french', 'spanish',  'irish', 'italian', 'arabic',  'armenian', 'chinese',
      'hebrew', 'japanese', 'polish', 'russian', 'swedish', 'welsh',  'norwegian'
   ];
...
sub addHelp {
   ...
   $self->addHelpItem( "[plugin_description]",
      "Translate text.  Usage: command <text>. See help Translator for a list of commands.  Use the -q option to quiet the intro text." );
}
